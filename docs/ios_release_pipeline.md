...
| 方式 | 适用 | 操作 |
|---|---|---|
| 内部测试 | 自己/开发团队（≤100 人，几分钟可用，免审核） | TestFlight → 内部测试 →「+」建组 → 添加成员（**成员须是 ASC 用户管理里的成员**，如开发者账号）→ 分配构建，自动发邮件 |
| 外部测试 | 其他人（朋友/小范围用户，需 Beta 审核 1-3 天） | 外部测试 → 新建群组 → 添加测试者邮箱 → 选构建 → 提交审核（填简短测试说明，如"冥想应用，含音频播放"）→ 通过后可生成**公开链接**直接分享 |

测试者手机安装：

1. 手机装**官方 TestFlight App**（蓝色图标），用**收到邀请的邮箱**对应的 Apple ID 登录
2. 点邀请邮件里的链接（内部）/ 打开公开链接点「注册测试」（外部）→ 接受邀请
3. TestFlight 里出现 Mindra 后点安装；首次启动弹权限请求（麦克风/音频）正常允许

注意事项：

- **外部测试的 Beta 审核也查出口合规**——旧构建要先手动处理（见上），带 Info.plist 声明的新构建自动通过
- 手机装过车道 B 的 AdHoc 包（同 bundle id）时，装 TestFlight 版可能提示卸载旧版，先卸载再装
- 上传 TestFlight 失败的处理在下方故障排查（build number 重复、出口合规等）
